"""
Multi-size CT phantom + CNR demo (with quantum Poisson noise & contrast)
-----------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# 6.  Main driver – computes CNR & contrast from the *reconstructed* image
# ---------------------------------------------------------------------

def demo(N: int = 512, E_keV: float = 70.0, I0: float | None = 5e4,
         show_images: bool = True, window_hu: int = 400):
    """Run the phantom simulation and print a CNR / contrast table."""
    ...
    df = (pd
          ...
          .rename_axis("Tissue")
          .round({"HU": 1, "CNR_vs_muscle": 2, "Contrast_vs_muscle": 3}))

    # extra lines for tumour‑to‑nerve metrics
    df.loc["tumor‑to‑nerve CNR"]      = [np.nan, round(cnr_tumor_nerve, 2), np.nan]
    df.loc["tumor‑to‑nerve contrast"] = [np.nan, np.nan, round(contrast_tumor_nerve, 3)]
