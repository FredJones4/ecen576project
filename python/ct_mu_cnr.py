...
★ Added μ column (linear‑attenuation coefficient) and μ‑based CNR column to
  the metrics table, alongside a renamed HU‑based CNR column for clarity.
---------------------------------------------------------------------
'''
...
def add_poisson_noise(sino_px: np.ndarray,
                      I0: float = 1e5,
                      px_cm: float = None) -> np.ndarray:
    """Add quantum (shot) noise to a noiseless sinogram in pixel units."""
    # 1) convert back to a physical line integral (cm⁻¹·cm)
    sino_cm = sino_px * px_cm

    # 2) simulate photon counts
    expected = I0 * np.exp(-sino_cm)           # now exp(-∫μ dl)
    noisy     = np.random.poisson(lam=expected).astype(float)

    # 3) avoid zeros, form the measured log-sinogram (cm⁻¹·cm)
    ratio = noisy / I0
    ratio[ratio <= 0] = EPS
    ...
    # 4) convert back to radon’s pixel-unit sinogram
...
def ct_pipeline(mu: np.ndarray, *, angles=None, filter_name: str = 'ramp',
                circle: bool = True, I0: float | None = None, px_cm=None):
    ...
    if I0 is not None:
        sino = add_poisson_noise(sino, I0, px_cm)  # TODO: uncomment to enable noise
...
# ---------------------------------------------------------------------
# 6.  Main driver – computes CNR, SNR, contrast & μ‑metrics
# ---------------------------------------------------------------------
...
def demo(N: int = 512, E_keV: float = 70.0, I0: float | None = 5e4,
    ...
    mu_water = 0.19  # cm⁻¹ – keep consistent across conversions
    ...
    sino, bp, fbp = ct_pipeline(mu_img, filter_name='ramp', I0=I0, px_cm=px_cm)
    ...
    # Noise estimate in HU and conversion to μ ------------------------
    sigma_hu = hu_img[masks["muscle"]].std(ddof=1)# was (np.sqrt(np.abs(mean_hu["muscle"]))) but I belive this other will go pixel by pixel 
    sigma_mu = sigma_hu * mu_water / 1000.0  # since HU = 1000*(μ-μw)/μw

    # SNR per tissue (HU) --------------------------------------------
    snr = {t: mean_hu[t] / sigma_hu if sigma_hu != 0 else np.inf for t in mean_hu}

    # CNRs ------------------------------------------------------------
    hu_cnr_to_muscle = {t: np.abs(mean_hu[t] - mean_hu["muscle"]) / sigma_hu if sigma_hu != 0 else np.inf
                        for t in ["fat", "tumor", "nerve"]}
    hu_cnr_tumor_nerve = np.abs(mean_hu["tumor"] - mean_hu["nerve"]) / sigma_hu if sigma_hu != 0 else np.inf

    mu_cnr_to_muscle = {t: np.abs(mean_mu[t] - mean_mu["muscle"]) / sigma_mu if sigma_mu != 0 else np.inf
                        for t in ["fat", "tumor", "nerve"]}
    mu_cnr_tumor_nerve = np.abs(mean_mu["tumor"] - mean_mu["nerve"]) / sigma_mu if sigma_mu != 0 else np.inf
    ...
    df = (pd
          ...
          .round({
              ...
              'HU_CNR_vs_muscle': 2,
              'mu_CNR_vs_muscle': 2,
              'Contrast_vs_muscle': 3
          }))

    # extra lines for tumour‑to‑nerve metrics
    df.loc["tumor‑to‑nerve HU_CNR"] = [np.nan, np.nan, np.nan, round(hu_cnr_tumor_nerve, 2), round(mu_cnr_tumor_nerve, 2), np.nan]
    df.loc["tumor‑to‑nerve mu_CNR"] = [np.nan, np.nan, np.nan, np.nan, round(mu_cnr_tumor_nerve, 2), np.nan]
    df.loc["tumor‑to‑nerve contrast"] = [np.nan, np.nan, np.nan, np.nan, np.nan, round(contrast_tumor_nerve, 3)]
