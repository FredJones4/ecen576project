...
# -------------------------------------------------
TR_ms        = 3000  # repetition time in ms
...
print(f"Estimated noise sigma: {sigma_noise:.4f}")
...
# ---- Rician Noise: Mean Noisy Signal vs TE ----------------------------------
fig = window.add_figure_tab('Mean Noisy Signal vs TE', col=0)
ax = fig.add_subplot()
n_samples = 5000
for tissue, sig in signals.items():
    # Monte Carlo mean of Rician magnitude
    noisy_means = [
        np.mean(np.sqrt((val + np.random.normal(0, sigma_noise, n_samples))**2 +
                         (np.random.normal(0, sigma_noise, n_samples))**2))
        for val in sig
    ]
    ax.plot(TE_values_ms, noisy_means, '--', label=f"{tissue.capitalize()} (noisy)")
ax.set(xlabel='Echo time TE (ms)', ylabel='Mean noisy signal',
       title='Mean Rician-Distributed Signal vs TE')
ax.grid(True)
ax.legend()

# ---- Rician Noise: Histogram at Mid-TE --------------------------------------
fig = window.add_figure_tab('Rician Noise Histogram', col=1)
ax = fig.add_subplot()
mid_idx = len(TE_values_ms) // 2
signal_mid = signals['tumor'][mid_idx]
hist_samples = np.sqrt((signal_mid + np.random.normal(0, sigma_noise, n_samples))**2 +
                       (np.random.normal(0, sigma_noise, n_samples))**2)
ax.hist(hist_samples, bins=60)
ax.set(xlabel='Intensity', ylabel='Count',
       title=f'Rician Histogram at TE={TE_values_ms[mid_idx]:.1f} ms')
ax.grid(True)
