...
nTRs_list    = np.array([256, 512, 1024, 2048])
...
# -------------------------
# New: Compute CNR Tumor vs Muscle
# -------------------------
cnr_tumor_muscle_opt = np.abs(signals_opt['tumor'] - signals_opt['muscle']) / sigma_noise_opt
...
# CNR: Tumor vs Muscle (Optimal)
fig = window.add_figure_tab('CNR Tumor/Muscle (opt)', col=0)
ax = fig.add_subplot()
ax.plot(TE_values_opt, cnr_tumor_muscle_opt)
ax.set(xlabel='TE (ms)', ylabel='CNR',
       title=f'CNR Tumor vs Muscle, TR={TR_opt} ms')
ax.grid(True)

# Optimization summary as a figure tab
fig = window.add_figure_tab('Optimization Summary', col=1)
ax = fig.add_subplot()
ax.axis('off')  # no axes
summary = f"""
Optimal Parameters:
...
Mean CNR Tumor→Muscle = {np.mean(cnr_tumor_muscle_opt):.3f}
"""
ax.text(
    0.01, 0.99,
    summary,
